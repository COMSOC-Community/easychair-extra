...
class TestRead(TestCase):
    ...
    def test_read_submission(self):
        ...
        subs = read_submission(os.path.join(root_dir, "submission.csv"), remove_deleted=False)
        assert len(subs[subs["deleted?"] == "yes"].index) > 0
        subs = read_submission(os.path.join(root_dir, "submission.csv"), remove_desk_reject=False)
        assert len(subs[subs["decision"] == "desk reject"].index) > 0
